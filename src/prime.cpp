...
void PrintCompactStatistics(double totalRuntime)
{
    ...
    for (unsigned int i = 3; i < nMaxChainLength; i++)
    {
        ...
        {
            double dChainsPerHour = ((double)vTotalChainsFound[i] / totalRuntime) * 3600000.0;
            strOutput += strprintf(" %uch/h: %4.3f ", i + 1, dChainsPerHour);
        }
    }
    printf("%s\n", strOutput.c_str());
}
