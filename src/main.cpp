...
double dTotalChainsExpected = 0.0;
        ...
        loop
        {
            ...
            if (nMillisNow - nHPSTimerStart > 60000)
            {
                ...
                if (nMillisNow - nHPSTimerStart > 60000)
                {
                    ...
                    nHPSTimerStart = nMillisNow;
                    double dPrimesPerSec = 1000.0 * nPrimeCounter / nTimeDiffMillis;
                    double dTestsPerSec = 1000.0 * nTestCounter / nTimeDiffMillis;
                    dChainsPerDay = 86400000.0 * dTotalChainsExpected / totalRunTime;
                    ...
                    if (nMillisNow - nLogTime > 59000)
                    {
                        nLogTime = nMillisNow;
                        if (fLogTimestamps)
                            printf("-- MINING: %6.0f prime/s %6.0f test/s  %3.8f chain/d  %3.8f block/d  primorial: %u  run: %3.2f,\n", dPrimesPerSec, dTestsPerSec, dChainsPerDay, dBlocksPerDay, nPrimorialMultiplier, totalRunTime/(3600*1000));
                        else
                            printf("%s -- MINING: %7.0f prime/s %7.0f test/s  %3.8f chain/d  %3.8f block/d  primorial: %u  run: %3.2f,\n", DateTimeStrFormat("%Y-%m-%d %H:%M:%S", nLogTime / 1000).c_str(), dPrimesPerSec, dTestsPerSec, dChainsPerDay, dBlocksPerDay, nPrimorialMultiplier, totalRunTime/(3600*1000));
            ...
            if (fNewBlock)
            {
                ...
                dTotalChainsExpected += dRoundChainExpected;
